...
class SSIM(Base):
    ...
    def __call__(self, im1, im2, multiscale=False):
        ...
        im1 = torch.tensor(im1).unsqueeze(dim=0).permute(0, 3, 1, 2).type(torch.float32)    # for pytorch_ssim calculation!
        im2 = torch.tensor(im2).unsqueeze(dim=0).permute(0, 3, 1, 2).type(torch.float32)
