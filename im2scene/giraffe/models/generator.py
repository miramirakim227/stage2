...
from .model import ViTEncoder, ResEncoder
...
class Generator(nn.Module):
    ...
    def __init__(self, device, z_dim=256, z_dim_bg=128, decoder=None,
                 ...
                 img_size=None, cam_weight = None, **kwargs):
        super().__init__()
        self.device = device
        ...
        self.resnet = ResEncoder(feature_size=img_size // 32)
    ...
    def forward(self, img, pose, batch_size=32, latent_codes=None, camera_matrices=None,
        ...
        scale, trans, rotmat, shape, appearance = self.resnet(img) # 생성된 애에서 rgb 뽑음 
        latent_codes = shape, appearance
        pred_cam = scale, trans, rotmat

        # shapeNet과 Carla는 scaling없이 rotation과 translation으로만 정직하게 구성된 데이터셋!
        # 생각해보니.. shapenet도 이거에 해당하나..? -> extrinsic matrix가 주어졌을 때 이렇게 분해해본 적이 있는가
        homo_ = torch.tensor([0, 0, 0, 1]).reshape(1, -1, 4).repeat(len(rotmat), 1, 1).to(self.device)
        camera_mat = torch.cat([torch.cat([rotmat, trans.unsqueeze(-1)], dim=-1), homo_], dim=1)
        scale_mat = torch.eye(4).unsqueeze(0).repeat(len(rotmat), 1, 1).to(self.device)
        ...
        if mode == 'val':
            ...
            shape_swap_rgb_v = self.volume_render_image(
                ...
                mode=mode, it=it, not_render_background=not_render_background,
                only_render_background=only_render_background)
            shape_rgb = self.neural_renderer(shape_swap_rgb_v)

            # app swap 
            app_swap_latent = latent_codes[0], latent_codes[1].flip(0)
            app_swap_rgb_v = self.volume_render_image(
                app_swap_latent, camera_matrices, 
                mode=mode, it=it, not_render_background=not_render_background,
                only_render_background=only_render_background)
            app_rgb = self.neural_renderer(app_swap_rgb_v)

            # pose swap 
            swap_pose_rgb_v = self.volume_render_image(
                latent_codes, swap_camera_matrices, 
                mode=mode, it=it, not_render_background=not_render_background,
                only_render_background=only_render_background)
            pose_rgb = self.neural_renderer(swap_pose_rgb_v)
            return rgb, shape_rgb, app_rgb, pose_rgb, pred_cam
        else:
