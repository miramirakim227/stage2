...
metric_val_best_psnrAE = load_dict.get(
    'loss_val_best_psnrAE', -model_selection_sign * np.inf)

if metric_val_best_psnrAE == np.inf or metric_val_best_psnrAE == -np.inf:
    metric_val_best_psnrAE = -model_selection_sign * np.inf


print(f'Current best validation metric psnrAE : {metric_val_best_psnrAE}')


logger = SummaryWriter(os.path.join(out_dir, 'logs'))
logger_py.info(f'NAME: {cfg["training"]["out_dir"]} \n Major settings: \n 0) resolution: {cfg["data"]["img_size"]} \n 1) LR_G: {cfg["training"]["learning_rate"]} \n 2) LR_D: {cfg["training"]["learning_rate_d"]} \n 3) range_u: {cfg["training"]["range_u"]} \n 4) range_v: {cfg["training"]["range_v"]} \n 5) recon_weight: {cfg["training"]["recon_weight"]} \n 6) cam_weight: {cfg["training"]["cam_weight"]}')
...
while (True):
    ...
    for batch in train_loader:
        ...
        if validate_every > 0 and (it % validate_every) == 0 and (it > 0):      # 여기서 validation까지도 전부 계산하기
            ...
            # 여기를 마지막에 val dataset으로 바꾸기
            eval_dict = trainer.evaluate(batch)     # 어차피 얘가 찐 eval은 아니니까..!
            logger_py.info(f'Validation metric = psnrAE:  {eval_dict["psnrAE"]} | ssim :  {eval_dict["ssimAE"]} | lpips : {eval_dict["lpipsAE"]}')                
                           
            for k, v in eval_dict.items():
                logger.add_scalar('val/%s' % k, v, it)

            metric_val = eval_dict['psnrAE']  # psnrAE
            if model_selection_sign * (metric_val - metric_val_best_psnrAE) > 0:
                metric_val_best_psnrAE = metric_val
                logger_py.info('New best model psnrAE (loss %.4f)' % metric_val_best_psnrAE)
                checkpoint_io.backup_model_best('model_best_psnrAE.pt')
                checkpoint_io.save('model_best_psnrAE.pt', epoch_it=epoch_it, it=it,
                                   loss_val_best_psnrAE=metric_val_best_psnrAE)
